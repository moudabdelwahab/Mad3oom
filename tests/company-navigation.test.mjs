...
test('لا ترحيل جديد بلا قرار صريح', () => {
    ...
    //   051 — زر «إرفاق فاتورة» (مطلوب صراحةً من صاحب المنتج: الاشتراك يذهب
    //         أولًا إلى acc، ثم يُرفق المالك أو الأدمن أو الموظف الفاتورة
    //         في ردود التذكرة بزر). الإرفاق كان داخل record_accounting_invoice
    //         تلقائيًا، وفصله وفحص صلاحية الزر (is_platform_staff) مكانهما
    //         القاعدة لا الواجهة.
    assert.equal(migrations[migrations.length - 1], '051_invoice_attach_button.sql',
