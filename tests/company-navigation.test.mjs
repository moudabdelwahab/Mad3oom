...
test('لا ترحيل جديد بلا قرار صريح', () => {
    ...
    //   036 — بوابة إصدار مفاتيح API. الدالة المنشورة تُنشئ مفتاحًا لأي حساب
    //         مسجّل بأي صلاحية، ومنها admin:full. سقف الصلاحيات لا يمكن أن
    //         يُفرَض من الواجهة، فمكانه القاعدة.
    //   037 — بوابة إنشاء الحسابات. الترحيل 035 قاعد super_user فانكسر مسار
    //         إضافة عضو الشركة، والنسخة المُحصَّنة الأولى أغفلت مسار طاقم
    //         المنصة فكانت ستكسر لوحة الإدارة. المساران يُقرَّران في القاعدة.
    //   038 — سلطة المنصة كبيانات (platform_authority) والسياق كمُرشِّح عليها.
    //         التدقيق أثبت أن معنى «حساب مرتفع السلطة» لم يكن مخزَّنًا في أي
    //         علاقة: ستة حسابات تحمل 'admin' ولا شيء يميّز الاثنين، و
    //         custom_roles جدول صلاحيات عام بصفر صفوف لم يُستعمل قط.
    //   039 — زرع المالك والحسابين المرتفعين بمعرّفات. يسبق 040 عمدًا حتى لا
    //         تمرّ لحظة يفقد فيها support@/info@ سلطتهما بنزع البريد.
    //   040 — نزع profiles.email من خمس دوال تفويض. العمود قابل للكتابة من
    //         العميل (027)، والمانع الوحيد كان تعارض مفتاح فريد — حظّ لا
    //         حاجز. لا يمكن أن يكون تغيير واجهة: 76 سياسة RLS تعتمد عليها.
    //   041 — احتواء معاينة عضو الشركة. المالك يظل مالكًا بحكم العلاقة داخل
    //         المعاينة، فيرى تذاكر أعضائه ورموزهم — تصعيد صلاحية لا تُصلحه
    //         الواجهة، ومقيسٌ بضابط سلبي في platform-owner-context.test.sql.
    //   042 — بوابة الحساب: هاتف إلزامي وفريد، وقائمة انتظار، وكود مرور
    //         بديل. مطلوب صراحةً من صاحب المنتج. ولا يمكن أن يكون تغيير
    //         واجهة بحال: المنع نفسه هو المطلوب، وأي فحص في المتصفح
    //         يتجاوزه نداء PostgREST مباشر. لذلك الفرض سياسات RESTRICTIVE
    ...
    //   043 — حارس أعمدة api_tokens. RLS تحرس الصفوف لا الأعمدة، وسياسة
    //         التحديث كانت تسمح لصاحب المفتاح بتحديث صفّه كاملًا — فيرفع
    //         scopes إلى admin:full، أو يمسح expires_at، أو يعيد إسناد
    //         user_id. لا يمكن أن يكون تغيير واجهة: نداء PostgREST مباشر
    //         يتجاوز أي فحص في المتصفح. (C-03)
    //   044 — سلامة حالة site_errors. مسار الإبلاغ عن الأخطاء مفتوح لـanon
    //         عمدًا (وهو صحيح)، لكن السياسة لم تقيّد status ولا الأطوال ولا
    //         type — فكان بإمكان أي زائر حشو الجدول وتزييف حالة 'resolved'
    //         فتختفي أخطاء حقيقية من لوحة الإدارة. (C-06)
    //   045 — إعدادات فوترة إرسال واتساب (wallet_enforce_send افتراضيًا
    //         false، و text_charge_egp). قرار صريح من صاحب المنتج: **قياس
    //         أولًا ثم حظر** — محفظة واحدة من ثلاث ممولة، والفرض الفوري كان
    //         سيقطع الإرسال عن مستأجرَين. (H-03)
    //   046 — تقوية التحقق من رمز تيليجرام. ثلاثة أشياء: إسقاط سياسة القراءة
    //         عن admin_telegram_otps (صاحب الحساب كان يقرأ otp_hash — وهو
    //         SHA-256 لستة أرقام يُكسر محليًا، فيسقط العامل الثاني أمام من
