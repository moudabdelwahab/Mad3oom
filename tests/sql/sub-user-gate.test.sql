-- ============================================================================
-- اختبار تنفيذي لـ migrations/037_sub_user_create_gate.sql
--
-- مساران مشروعان لا مسار واحد:
--   طاقم المنصة  → حساب مستقل (attach_to_company = false)
--   مدير الشركة  → عضو تابع   (attach_to_company = true)
--
-- والمصفوفة كاملة: company_user يُمنع · شركة بلا استحقاق تُمنع · عميل بلا
-- شركة يُمنع · مجهول يُمنع · تزوير المعرّفات مستحيل بنيويًا.
--
...
\i migrations/037_sub_user_create_gate.sql
-- ============================================================================

-- شركة ب: اشتراك بلا ميزة sub_users
INSERT INTO public.subscription_plans (key, name_ar) VALUES ('basic','الأساسية');
INSERT INTO public.feature_flags (key, name_ar) VALUES ('analytics','تحليلات');
INSERT INTO public.plan_features (plan_id, feature_key)
  SELECT id,'analytics' FROM public.subscription_plans WHERE key='basic';
...
-- ============================================================================
-- ① مسار طاقم المنصة — حساب مستقل، كما كان دائمًا
-- ============================================================================
SET request.jwt.claim.sub = '99999999-9999-4999-8999-999999999999';   -- admin
DO $$
DECLARE v jsonb;
BEGIN
  v := public.sub_user_create_context();
  IF (v->>'allowed')::boolean IS NOT TRUE THEN
    RAISE EXCEPTION 'طاقم المنصة مُنع من إنشاء حساب — انحدار في لوحة الإدارة';
  END IF;
  IF (v->>'actor') <> 'platform_staff' THEN RAISE EXCEPTION 'actor خاطئ: %', v->>'actor'; END IF;
  IF (v->>'attach_to_company')::boolean IS NOT FALSE THEN
    RAISE EXCEPTION 'حساب الأدمن سيُربط بشركة — سلوك جديد غير مقصود';
  END IF;
  RAISE NOTICE 'PASS SU-1: طاقم المنصة يُنشئ حسابًا مستقلًا (السلوك القديم محفوظ)';
END $$;

-- وحتى لو ملك الأدمن شركة، يظل مساره إداريًا لا مسار شركة
SET request.jwt.claim.sub = '44444444-4444-4444-8444-444444444444';   -- admin يملك شركة د
DO $$
DECLARE v jsonb;
BEGIN
  IF NOT public.owns_a_company('44444444-4444-4444-8444-444444444444') THEN
    RAISE EXCEPTION 'التمهيد خاطئ: هذا الأدمن لا يملك شركة';
  END IF;
  v := public.sub_user_create_context();
  IF (v->>'actor') <> 'platform_staff' THEN
    RAISE EXCEPTION 'أدمن يملك شركة سلك مسار الشركة: %', v->>'actor';
  END IF;
  IF (v->>'attach_to_company')::boolean IS NOT FALSE THEN
    RAISE EXCEPTION 'أدمن يملك شركة رُبط حسابه الجديد بها';
  END IF;
  RAISE NOTICE 'PASS SU-2: الأدمن يظل مسارًا إداريًا ولو ملك شركة';
END $$;

-- ============================================================================
-- ② مسار مدير الشركة — عضو تابع
-- ============================================================================
...
BEGIN
  v := public.sub_user_create_context();
  IF (v->>'allowed')::boolean IS NOT TRUE THEN
    RAISE EXCEPTION 'مدير شركة مستحق مُنع من إضافة عضو';
  END IF;
  IF (v->>'actor') <> 'company_admin' THEN RAISE EXCEPTION 'actor خاطئ: %', v->>'actor'; END IF;
  IF (v->>'attach_to_company')::boolean IS NOT TRUE THEN
    RAISE EXCEPTION 'عضو الشركة لن يُربط بها — المسار الثاني مكسور';
  END IF;
  RAISE NOTICE 'PASS SU-3: مدير الشركة يُنشئ عضوًا تابعًا لشركته';
END $$;

-- ============================================================================
-- ③ المصفوفة: من يُمنع
-- ============================================================================
SET request.jwt.claim.sub = '1a1a1a1a-1a1a-4a1a-8a1a-1a1a1a1a1a1a';   -- company_user
DO $$
DECLARE v jsonb;
BEGIN
  v := public.sub_user_create_context();
  IF (v->>'allowed')::boolean IS NOT FALSE THEN RAISE EXCEPTION 'company_user أنشأ حسابًا'; END IF;
  IF (v->>'actor') <> 'company_user' THEN RAISE EXCEPTION 'actor خاطئ: %', v->>'actor'; END IF;
  RAISE NOTICE 'PASS SU-4: company_user لا يُنشئ حسابات — العضوية ليست ترقية';
END $$;

SET request.jwt.claim.sub = '22222222-2222-4222-8222-222222222222';   -- مدير شركة بلا sub_users
DO $$
DECLARE v jsonb;
BEGIN
  IF NOT public.is_company_admin() THEN RAISE EXCEPTION 'التمهيد خاطئ: ليس مدير شركة'; END IF;
  IF public.company_has_feature('sub_users') THEN RAISE EXCEPTION 'التمهيد خاطئ: لديه الاستحقاق'; END IF;
  v := public.sub_user_create_context();
  IF (v->>'allowed')::boolean IS NOT FALSE THEN
    RAISE EXCEPTION 'شركة بلا استحقاق sub_users أضافت عضوًا';
  END IF;
  IF (v->>'actor') <> 'company_admin_no_entitlement' THEN
    RAISE EXCEPTION 'تصنيف الرفض خاطئ: %', v->>'actor';
  END IF;
  RAISE NOTICE 'PASS SU-5: بلا استحقاق sub_users لا إضافة';
END $$;

SET request.jwt.claim.sub = '55555555-5555-4555-8555-555555555555';   -- عميل بلا شركة
DO $$
DECLARE v jsonb;
BEGIN
  v := public.sub_user_create_context();
  IF (v->>'allowed')::boolean IS NOT FALSE THEN RAISE EXCEPTION 'عميل بلا شركة أنشأ حسابًا'; END IF;
  IF (v->>'actor') <> 'customer' THEN RAISE EXCEPTION 'actor خاطئ: %', v->>'actor'; END IF;
  RAISE NOTICE 'PASS SU-6: عميل بلا شركة لا يُنشئ حسابات';
END $$;
...
BEGIN
  v := public.sub_user_create_context();
  IF (v->>'allowed')::boolean IS NOT FALSE THEN RAISE EXCEPTION 'مجهول أنشأ حسابًا'; END IF;
  IF (v->>'actor') <> 'anonymous' THEN RAISE EXCEPTION 'actor خاطئ للمجهول'; END IF;
  IF (v->>'attach_to_company')::boolean IS NOT FALSE THEN RAISE EXCEPTION 'مجهول سيُربط بشركة'; END IF;
  RAISE NOTICE 'PASS SU-7: المجهول ممنوع';
END $$;

-- ============================================================================
-- ④ تزوير الهوية مستحيل بنيويًا
-- ============================================================================
DO $$
DECLARE v_src text;
BEGIN
  v_src := pg_get_functiondef('public.sub_user_create_context()'::regprocedure);
  IF v_src ~ 'p_user_id|p_company_id|p_owner_id' THEN
    RAISE EXCEPTION 'البوابة تقبل معرّف هوية كمُعامل — قابل للتزوير';
  END IF;
  IF v_src !~ 'auth\.uid\(\)' THEN
    RAISE EXCEPTION 'البوابة لا تشتقّ الهوية من auth.uid()';
  END IF;
  RAISE NOTICE 'PASS SU-8: لا مُعامل هوية — النطاق من auth.uid() وحده';
END $$;

-- ⑤ anon محجوب على مستوى الصلاحيات
DO $$
BEGIN
  IF has_function_privilege('anon', 'public.sub_user_create_context()'::regprocedure, 'EXECUTE') THEN
    RAISE EXCEPTION 'anon يملك تنفيذ بوابة إنشاء الحسابات';
  END IF;
  RAISE NOTICE 'PASS SU-9: البوابة محجوبة عن anon';
END $$;

SELECT 'ALL SUB-USER GATE TESTS PASSED';
