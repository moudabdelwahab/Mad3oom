-- ============================================================================
-- اختبار تنفيذي لـ 052: سلطة مالك المنصة المطلقة على SIE.
--
-- يثبّت، كل خاصية تفشل إن انكسرت:
--
--   ① المالك مدير SIE وطاقم المحرك في كل سياق — وبلا سياق أصلًا
--   ② …عدا معاينة عضو الشركة (عقيدة 041 لا تُكسر)
--   ③ المالك يشغّل/يوقف المحرك ويكتب الإعدادات ووصول العملاء عبر RLS الحقيقية
--   ④ support@ يحتفظ بصلاحيته (منح مزروع) — ولا أحد غيره يكسب شيئًا
--   ⑤ البريد لم يعد سلطة: تغييره إلى support@ لا يمنح شيئًا
--   ⑥ المنح والسحب للمالك وحده، والكتابة المباشرة على الجداول مرفوضة
--   ⑦ منح لا يسري (حساب عميل) يُرفض، والسجل غير قابل للتعديل
--   ⑧ «السلطة والمنح»: المالك يرى كل صفوف السلطة داخل سياقه، وغيره لا
--
-- التمهيد (الجداول، دوال 035، RLS، الفاعلون، و038→041) منسوخ من
-- platform-owner-context.test.sql حتى يطبَّق 052 على نفس حالة الإنتاج.
-- ============================================================================
...
-- ============================================================================
-- ١) المالك يملك SIE بلا سياق، وفي كل سياق عدا المعاينة
-- ============================================================================
SET request.jwt.claim.sub = '11111111-1111-4111-8111-111111111111';
...
BEGIN
  ...
  IF public.is_sie_admin() IS DISTINCT FROM true THEN RAISE EXCEPTION 'المالك بلا سياق ليس مدير SIE'; END IF;
  IF public.is_chat_engine_staff() IS DISTINCT FROM true THEN RAISE EXCEPTION 'المالك بلا سياق ليس طاقم المحرك'; END IF;
  -- وسلطة المنصة الأخرى ما زالت fail-closed بلا سياق — 052 لا يوسّعها
  IF public.is_admin() IS DISTINCT FROM false THEN RAISE EXCEPTION '052 وسّع is_admin بلا سياق'; END IF;

  FOREACH c IN ARRAY ARRAY['owner','admin','customer','company_admin'] LOOP
    PERFORM public.enter_context(c);
    IF public.is_sie_admin() IS DISTINCT FROM true OR public.is_chat_engine_staff() IS DISTINCT FROM true THEN
      RAISE EXCEPTION 'المالك فقد SIE في سياق %', c;
    END IF;
  END LOOP;
  RAISE NOTICE 'PASS 1: المالك مدير SIE وطاقم المحرك بلا سياق وفي كل واجهة';
END $$;

-- ٢) معاينة عضو الشركة ⊆ العضو الحقيقي — والعضو لا يملك شيئًا من SIE
DO $$
BEGIN
  PERFORM public.enter_context('company_user_preview');
  IF NOT public.preview_mode() THEN RAISE EXCEPTION 'التمهيد خاطئ: المعاينة لم تُفعَّل'; END IF;
  IF public.is_sie_admin() IS DISTINCT FROM false OR public.is_chat_engine_staff() IS DISTINCT FROM false
     OR public.sie_owner_authority() IS DISTINCT FROM false THEN
    RAISE EXCEPTION 'المعاينة منحت صلاحية SIE لا يملكها العضو الحقيقي';
  END IF;
  BEGIN
    PERFORM public.owner_grant_sie_admin('44444444-4444-4444-8444-444444444444', 'من المعاينة');
    RAISE EXCEPTION 'FAIL: منح من داخل المعاينة';
  EXCEPTION WHEN insufficient_privilege THEN NULL;
  END;
  PERFORM public.exit_context();
  RAISE NOTICE 'PASS 2: لا SIE داخل معاينة العضو';
END $$;

-- ٣) التشغيل الفعلي عبر RLS الحقيقية — بلا سياق
DO $$
DECLARE v int;
BEGIN
  IF public.active_context() IS NOT NULL THEN RAISE EXCEPTION 'التمهيد خاطئ: سياق باقٍ'; END IF;
  UPDATE public.sie_settings SET value = 'false'::jsonb WHERE key = 'engine_enabled';
  GET DIAGNOSTICS v = ROW_COUNT;
  IF v <> 1 THEN RAISE EXCEPTION 'المالك لم يستطع إيقاف المحرك (% صف)', v; END IF;
  UPDATE public.sie_settings SET value = 'true'::jsonb WHERE key = 'engine_enabled';
  INSERT INTO public.sie_settings (key, value) VALUES ('answer_directly', 'true'::jsonb);
  INSERT INTO public.customer_sie_access (user_id, is_enabled, message_quota)
  VALUES ('66666666-6666-4666-8666-666666666666', true, 100);
  SELECT count(*) INTO v FROM public.customer_sie_access;
  IF v <> 1 THEN RAISE EXCEPTION 'المالك لا يرى وصول العملاء'; END IF;
  RAISE NOTICE 'PASS 3: المالك يوقف/يشغّل المحرك ويدير الإعدادات ووصول العملاء';
END $$;
RESET ROLE;

-- ============================================================================
-- ٤) لا أحد غير المالك كسب شيئًا، وsupport@ لم يخسر شيئًا
-- ============================================================================
DO $$
DECLARE r record;
BEGIN
  FOR r IN SELECT * FROM (VALUES
      -- الحساب                                  sie_admin  staff
      ('22222222-2222-4222-8222-222222222222'::uuid, true,  true),   -- support@ (منح مزروع)
      ('33333333-3333-4333-8333-333333333333'::uuid, false, true),   -- info@ — لم يكن مدير SIE قط
      ('44444444-4444-4444-8444-444444444444'::uuid, false, true),   -- أدمن عادي
      ('55555555-5555-4555-8555-555555555555'::uuid, false, false),  -- عضو شركة
      ('66666666-6666-4666-8666-666666666666'::uuid, false, false)   -- عميل
    ) AS t(uid, sie_admin, staff)
  LOOP
    PERFORM set_config('request.jwt.claim.sub', r.uid::text, false);
    IF public.is_sie_admin() IS DISTINCT FROM r.sie_admin THEN
      RAISE EXCEPTION 'is_sie_admin(%) = % والمتوقع %', r.uid, public.is_sie_admin(), r.sie_admin;
    END IF;
    IF public.is_chat_engine_staff() IS DISTINCT FROM r.staff THEN
      RAISE EXCEPTION 'is_chat_engine_staff(%) = % والمتوقع %', r.uid, public.is_chat_engine_staff(), r.staff;
    END IF;
    IF public.sie_owner_authority() THEN RAISE EXCEPTION '% صار مالكًا لـSIE', r.uid; END IF;
  END LOOP;
  RAISE NOTICE 'PASS 4: support@ باقٍ، ولا حساب آخر اكتسب صلاحية';
END $$;

-- ٥) البريد لم يعد سلطة
UPDATE auth.users SET email = 'support@mad3oom.online' WHERE id = '66666666-6666-4666-8666-666666666666';
SET request.jwt.claim.sub = '66666666-6666-4666-8666-666666666666';
SET ROLE authenticated;
DO $$
DECLARE v int;
BEGIN
  IF public.is_sie_admin() THEN RAISE EXCEPTION 'بريد support@ منح إدارة SIE'; END IF;
  UPDATE public.sie_settings SET value = 'false'::jsonb WHERE key = 'engine_enabled';
  GET DIAGNOSTICS v = ROW_COUNT;
  IF v <> 0 THEN RAISE EXCEPTION 'عميل أوقف المحرك'; END IF;
  UPDATE public.customer_sie_access SET message_quota = 999999;
  GET DIAGNOSTICS v = ROW_COUNT;
  IF v <> 0 THEN RAISE EXCEPTION 'عميل عدّل كوتة SIE'; END IF;
  RAISE NOTICE 'PASS 5: البريد وحده لا يمنح شيئًا، والعميل لا يلمس المحرك';
END $$;
RESET ROLE;
UPDATE auth.users SET email = 'customer@example.com' WHERE id = '66666666-6666-4666-8666-666666666666';

-- ============================================================================
-- ٦) المنح والسحب — المالك وحده، ولا كتابة مباشرة
-- ============================================================================
SET request.jwt.claim.sub = '44444444-4444-4444-8444-444444444444';   -- أدمن عادي
SET ROLE authenticated;
DO $$
BEGIN
  BEGIN
    PERFORM public.owner_grant_sie_admin('44444444-4444-4444-8444-444444444444', 'ترقية ذاتية');
    RAISE EXCEPTION 'FAIL: أدمن منح نفسه إدارة SIE';
  EXCEPTION WHEN insufficient_privilege THEN NULL;
  END;
  BEGIN
    INSERT INTO public.sie_admin_grants (user_id) VALUES ('44444444-4444-4444-8444-444444444444');
    RAISE EXCEPTION 'FAIL: كتابة مباشرة في sie_admin_grants';
  EXCEPTION WHEN insufficient_privilege THEN NULL;
  END;
  BEGIN
    PERFORM public.owner_revoke_sie_admin('22222222-2222-4222-8222-222222222222');
    RAISE EXCEPTION 'FAIL: أدمن سحب منح غيره';
  EXCEPTION WHEN insufficient_privilege THEN NULL;
  END;
  IF (SELECT count(*) FROM public.sie_admin_grants) <> 0 THEN
    RAISE EXCEPTION 'أدمن عادي يقرأ منح غيره';
  END IF;
  RAISE NOTICE 'PASS 6a: غير المالك لا يمنح ولا يسحب ولا يكتب ولا يقرأ المنح';
END $$;
RESET ROLE;

SET request.jwt.claim.sub = '22222222-2222-4222-8222-222222222222';   -- support@ نفسه
SET ROLE authenticated;
DO $$
BEGIN
  BEGIN
    PERFORM public.owner_grant_sie_admin('44444444-4444-4444-8444-444444444444', null);
    RAISE EXCEPTION 'FAIL: مدير SIE منح غيره — المنح للمالك وحده';
  EXCEPTION WHEN insufficient_privilege THEN NULL;
  END;
  IF (SELECT count(*) FROM public.sie_admin_grants) <> 1 THEN
    RAISE EXCEPTION 'support@ لا يرى منحه هو';
  END IF;
  RAISE NOTICE 'PASS 6b: مدير SIE يدير العملاء ولا يمنح الإدارة';
END $$;
RESET ROLE;

SET request.jwt.claim.sub = '11111111-1111-4111-8111-111111111111';   -- المالك
SET ROLE authenticated;
DO $$
BEGIN
  PERFORM public.owner_grant_sie_admin('44444444-4444-4444-8444-444444444444', 'مسؤول تشغيل');
  PERFORM set_config('request.jwt.claim.sub', '44444444-4444-4444-8444-444444444444', false);
  IF NOT public.is_sie_admin() THEN RAISE EXCEPTION 'المنح لم يسرِ'; END IF;

  PERFORM set_config('request.jwt.claim.sub', '11111111-1111-4111-8111-111111111111', false);
  IF NOT public.owner_revoke_sie_admin('44444444-4444-4444-8444-444444444444') THEN
    RAISE EXCEPTION 'السحب لم يجد المنح';
  END IF;
  PERFORM set_config('request.jwt.claim.sub', '44444444-4444-4444-8444-444444444444', false);
  IF public.is_sie_admin() THEN RAISE EXCEPTION 'السحب لم يسرِ'; END IF;
  PERFORM set_config('request.jwt.claim.sub', '11111111-1111-4111-8111-111111111111', false);

  -- ⑦ منح لا يسري يُرفض بدل أن يُكتب
  BEGIN
    PERFORM public.owner_grant_sie_admin('66666666-6666-4666-8666-666666666666', null);
    RAISE EXCEPTION 'FAIL: مُنحت إدارة SIE لعميل';
  EXCEPTION WHEN invalid_parameter_value THEN NULL;
  END;

  IF (SELECT count(*) FROM public.sie_authority_audit WHERE action IN ('grant','revoke')) <> 2 THEN
    RAISE EXCEPTION 'السجل لا يطابق المنح والسحب';
  END IF;
  RAISE NOTICE 'PASS 6c: المالك يمنح ويسحب، والمنح يسري فورًا، ومنح العميل مرفوض';
END $$;
RESET ROLE;

-- ٧) السجل غير قابل للتعديل — ولا حتى من ترحيل
DO $$
BEGIN
  BEGIN
    DELETE FROM public.sie_authority_audit;
    RAISE EXCEPTION 'FAIL: حُذف سجل السلطة';
  EXCEPTION WHEN insufficient_privilege THEN NULL;
  END;
  RAISE NOTICE 'PASS 7: سجل منح SIE append-only';
END $$;

-- ============================================================================
-- ٨) «السلطة والمنح» — المالك يرى كل صفوف السلطة داخل سياقه فقط
-- ============================================================================
SET request.jwt.claim.sub = '11111111-1111-4111-8111-111111111111';
SET ROLE authenticated;
DO $$
...
BEGIN
  ...
  SELECT count(*) INTO v FROM public.platform_authority;
  IF v <> 1 THEN RAISE EXCEPTION 'خارج سياق المالك يرى % صفوف سلطة', v; END IF;
  PERFORM public.enter_context('owner');
  SELECT count(*) INTO v FROM public.platform_authority;
  IF v <> 3 THEN RAISE EXCEPTION 'في سياق المالك يرى % صفوف والمتوقع 3', v; END IF;
  BEGIN
    INSERT INTO public.platform_authority (user_id, level)
    VALUES ('44444444-4444-4444-8444-444444444444', 'elevated_admin');
    RAISE EXCEPTION 'FAIL: المالك كتب في platform_authority من جلسة';
  EXCEPTION WHEN insufficient_privilege THEN NULL;
  END;
  PERFORM public.exit_context();
  RAISE NOTICE 'PASS 8a: المالك يرى كل السلطة في سياقه، والكتابة للترحيل وحده';
END $$;
RESET ROLE;
...
DO $$
BEGIN
  IF (SELECT count(*) FROM public.platform_authority) <> 1 THEN
    RAISE EXCEPTION 'حامل السلطة المرتفعة يرى صفوف غيره';
  END IF;
  RAISE NOTICE 'PASS 8b: غير المالك يرى صفّه وحده';
END $$;
RESET ROLE;

SELECT 'ALL OWNER SIE AUTHORITY TESTS PASSED';
