...
async function handleGenerate(adminClient: any, catalog: Map<string, CatalogRow>, body: any, caller: Caller) {
    ...
    // الرسالة السابقة كانت "فشلت كل المحاولات (الأساسي وكل الاحتياطيات)" مهما
    // كان عدد المرشّحين — فتقرأ كأن سلسلة احتياطي كاملة جُرِّبت وسقطت، بينما
    // الواقع غالبًا مرشّح واحد بلا احتياطي معرَّف أصلًا. والأهم أنها كانت تبتلع
    // سبب الفشل الحقيقي: الأدمن يرى "فشلت كل المحاولات" ولا يرى أن رصيده نفد.
    const lastError = attempts[attempts.length - 1]?.error || "سبب غير معروف";
    const headline = chain.length === 1
        ? "فشل المزوّد المحدَّد، ولا يوجد احتياطي مُعرَّف لهذا الوضع"
        : `فشلت كل المحاولات (${chain.length} مزوّدين)`;

    return jsonResponse({
        success: false,
        request_id: requestId,
        error: `${headline} — ${lastError}`,
        headline,
        reason: lastError,
        // اقتراح مبني على الحالة: مرشّح واحد فقط يعني أن قاعدة توجيه واحدة
        // كانت ستنقذ الجلسة، وهي ميزة موجودة بالفعل لكن الأدمن قد لا يعرفها.
        suggestion: chain.length === 1
            ? "أضِف قاعدة في تبويب «التوجيه والاحتياطي» لهذا الوضع كي ينتقل النداء تلقائيًا لمزوّد آخر عند فشل الأساسي."
            : null,
        attempts,
    }, 502);
}
