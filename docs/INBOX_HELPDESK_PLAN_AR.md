...
> المرحلة 1 اتنفذت (`migrations/054_inbox_helpdesk_core.sql` + الواجهة + الاختبارات) و**لسه ماتطبّقتش على الإنتاج** — مستنية الموافقة (D5). راجع §5.
> كل الحقائق هنا مقروءة من قاعدة الإنتاج (قراءة فقط) ومن الكود، بتاريخ 2026-09-25.
...
|---|---|---|---|
| 1 | `054_inbox_helpdesk_core.sql` | دالة الوصول (D1) + الفرق + الإسناد + التحويل + الوسوم + الملاحظات + الأرشفة + السجل + `inbox_send_reply` / `inbox_close` | `tests/sql/inbox-helpdesk-core.test.sql` (على Postgres 16 المحلي — متاح في البيئة) + اختبارات عرض في المتصفح |
| 2 | `055_inbox_attachments_reactions_edits.sql` | المرفقات + سياسات التخزين + التفاعلات + التعديل/الحذف + تعديلات الويدجت | SQL + عرض الويدجت + الصندوق |
| 3 | `056_inbox_scheduled_replies.sql` | الجدولة + job الإرسال | SQL (بما فيها التنفيذ المتوازي و `skip locked`) + عرض |
...
**قبل الدمج:** الواجهة الجديدة بتعتمد على 054. لو الفرع اتدمج واتنشر قبل تطبيق 054، الصندوق هيعرض «حصل خطأ في تحميل المحادثات».
